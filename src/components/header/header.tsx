        ...
        <>
            <header className='w-full relative top-0, left-0, right-0 z-20'>
                ...
                sm:flex-row-reverse sm:justify-between sm:items-start'>
                    ...
                    <Link href={'/'}>
                        <Image className='w-64 h-auto sm:w-80 md:w-96' src={OfkorsLogo} alt="Logo Ofkors"/> 
                        {/* basic w-90 */}
                    </Link>
                </div>
                <nav className='w-full h-16 hidden text-black flex flex-row justify-end gap-x-5 items-center lg:flex'>
                    {MainMenu.map((item, index)=> {
                        return (
                            <Link className='transition ease-in-out mr-5 text-3xl font-medium text-[#47141e] hover:text-[#ff5f6d]' href={item.url} key={index}>
                                {item.label}
                            </Link>
                        )
                    })}
